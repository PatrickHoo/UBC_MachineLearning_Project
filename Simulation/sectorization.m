...
for Ru=2:0.2:10;  % this is the normalized Ru
    ...
    for i=1:1:10000; % repeat the process for 10,000 times
        ...
        gammab_sector=((((R*(Ru+1))/r)^a)*((g+(Ru+1)*R)/(g+r))^b)/2;%the best case of gammad
        gammaw_sector=((((R*(Ru-1))/r)^a)*((g+(Ru-1)*R)/(g+r))^b)/2;%the worst case of gammad
        %%%%%%%%%% STETP 6 %%%%%%%%%%%
        ...
        Aeb_sector(Ru_iteration,i)=(4/(pi*(Ru.^2)*(R*R)))*(log2(1+gammab_sector)); % ASE of the best cas interference
        Aew_sector(Ru_iteration,i)=(4/(pi*(Ru.^2)*(R*R)))*(log2(1+gammaw_sector)); % ASE ofthe worst cas interference
...
aAeb_sector=mean(Aeb_sector,2); %the average of the best case after running 10000 times
aAew_sector=mean(Aew_sector,2); %the average of the worst case after running 10000 times

Ru=2:0.2:10;
figure()
plot(Ru,aAeb,'g',Ru,aAew,'r',Ru,aAeb_sector,'g--',Ru,aAew_sector,'r--');%set the best case is green,the worst line is red
legend('Best-case','Worst-case','Best-case-3-Sectors','Worst-case-3-Sectors')
